...
def admin_dashboard():
    ...
    borrowed_books = {k: v for k, v in books.items() if 'borrowed_by' in v}
    return render_template('admin_dashboard.html', borrowed_books=borrowed_books)


@app.route('/admin/send_reminder/<book_id>', methods=['GET', 'POST'])
def send_reminder(book_id):
    if 'username' not in session or users[session['username']]['role'] != 'admin':
        return redirect(url_for('admin_login'))

    book = books.get(book_id)
    if not book or 'borrowed_by' not in book:
        flash('Book not found or not borrowed')
        return redirect(url_for('admin_dashboard'))

    if request.method == 'POST':
        user = book['borrowed_by']
        # Simulate sending a reminder (e.g., via email)
        flash(f'Reminder sent to {user} to return {book["title"]}')
        return redirect(url_for('admin_dashboard'))
